...
import { triggerAnalysis, getAnalysisJobs, getAnalysisJob, getJobReport } from './analysis.service';
...
export async function getJobById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const job = await getAnalysisJob(req.params.id, req.params.jobId, req.userId);
...
export async function getReport(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const report = await getJobReport(req.params.id, req.params.jobId, req.userId);
